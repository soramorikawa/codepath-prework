...
function generatePattern() { // generates a random 8 digit pattern(between 1 and 4)
  for (let i = 0; i < 8; i++) {
...
function guess(btn){
  ...
  if (btn == pattern[guessCounter]) { // correct guess
    if (guessCounter == progress) { // all correct guesses for the turn
      if (progress == pattern.length - 1) { // correct guesses for whole game -> win
        winGame();
      } else {
        progress++;
        playClueSequence();
